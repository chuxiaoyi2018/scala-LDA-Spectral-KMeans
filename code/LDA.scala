...
object LDA{
  ...
  def main(args: Array[String]): Unit = {
    ...
    val train_path = sc.textFile("E:/App/IDEA/workplace/ml/src/data/save_train_5000.txt")
      .filter(_.nonEmpty).map(_.split(" ").map(_.toDouble)).map(Vectors.dense).map(Row(_))
    val test_path = sc.textFile("E:/App/IDEA/workplace/ml/src/data/save_train_5000.txt")
      .filter(_.nonEmpty).map(_.split(" ").map(_.toDouble)).map(Vectors.dense).map(Row(_))
    val real_label = sc.textFile("E:/App/IDEA/workplace/ml/src/data/save_label_5000.txt")
